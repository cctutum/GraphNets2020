...
BASE_URL = "https://drive.google.com/drive/folders/11uuYl0peqPg2DQno64YPYMODPu8fjDXU?usp=sharing"
# updated with respect to the Medium-article

# subprocess.run(["mkdir", "-p", f"{OUTPUT_DIR}"], 
#                capture_output=True, text=True)

META_DATA_PATH = f"{OUTPUT_DIR}/metadata.json"
CLOUD_PATH = f"{BASE_URL}/metadata.json"
# subprocess.run(["wget", "-O", f"{META_DATA_PATH}", f"{CLOUD_PATH}"], 
#                capture_output=True, text=True)
...
"""
We need to go through the following steps to convert the raw data into graphs 
with descriptive node features and edge features:

1. Apply noise to the trajectory to have more diverse training examples,
2. Construct the graph based on the distance between particles,
3. Extract node-level features: particle velocities and their distance to the 
boundary,
4. Extract edge-level features: displacement and distance between particles.
"""
...
We will use this dataset in the training phase because the history of particles' 
states are necessary for the model to make predictions. But in the meantime, 
since long-horizon prediction is usually inaccurate and time-consuming, we sliced 
the trajectories to short time windows to improve the perfomance of the model.
"""
...
"""
Each data point in the dataset is a pyg.data.Data object which describes a graph. 
We explain the contents of the first data point, and visualize the graph.
"""
...
print( "Each node is a particle and each edge is the interaction between two particles.")
...
# Helper Class
"""
We first define a class for Multi-Layer Perceptron (MLP). This class generates 
an MLP given the width and the depth of it. Because MLPs are used in several 
places of the GNN, this helper class will make the code cleaner.
"""
...
subprocess.run(["mkdir", "-p", f"{model_path}"], 
               capture_output=True, text=True)
subprocess.run(["mkdir", "-p", f"{rollout_path}"], 
               capture_output=True, text=True)
...
#%%
### Load the checkpoint trained by us. 
### Do not run this block if you have trained your model in the previous block.
...
def visualize_pair(particle_type, position_pred, position_gt, metadata):
    ...
    return animation.FuncAnimation(fig, update, frames=100, interval=10, blit=True)
